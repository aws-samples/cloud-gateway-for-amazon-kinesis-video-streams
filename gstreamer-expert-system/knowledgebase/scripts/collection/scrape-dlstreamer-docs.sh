...
# Scrape DLStreamer Elements Documentation
set -e

echo "Scraping DLStreamer elements documentation..."

# Create target directory
mkdir -p docs/openvino/elements

# Scrape the main elements page
echo "Downloading DLStreamer elements guide..."
wget -O docs/openvino/elements/elements.html "https://dlstreamer.github.io/elements/elements.html" || true

# Try to get the full site structure
echo "Downloading additional DLStreamer documentation..."
wget --recursive --no-parent --convert-links --page-requisites \
     --no-host-directories --cut-dirs=1 \
     --directory-prefix="docs/openvino/dlstreamer-web" \
     --accept="html,pdf,md,txt" \
     --reject="*.js,*.css,*.png,*.jpg,*.gif,*.svg" \
     --wait=1 --random-wait \
     --timeout=30 --tries=2 \
     --level=2 \
     "https://dlstreamer.github.io/" || true

# Create a comprehensive elements reference
cat > docs/openvino/dlstreamer-elements-reference.md << 'EOF'
# DLStreamer GStreamer Elements Reference
...
echo "DLStreamer documentation scraping complete!"
echo "Files saved to docs/openvino/"
